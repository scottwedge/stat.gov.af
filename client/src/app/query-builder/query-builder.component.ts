...
export class QueryBuilderComponent implements OnInit, AfterViewInit {
	...
	constructor(
		...
		public datasouceQueryService: DatasourceQueryService,
	...
	ngOnInit() {
		...
		this.operators = {
			...
			flCn: (string, substring) => {
				return string.includes(substring);
			},
			flCnS: (string, substring) => {
				return string.startsWith(substring);
			},
			flCnE: (string, substring) => {
				return string.endsWith(substring);
			},
			flNCn: (string, substring) => {
				return !string.includes(substring);
			}
		};
	...
	getDatasets(customParams) {
		this.datasouceQueryService
			.getDatasets(customParams, 'dataset', 4000)
	...
	onFileChange(evt: any) {
		...
		reader.onload = (e: any) => {
			...
			/* save data */
			this.data = <Array<XLSX.AOA2SheetOpts>>(
			...
			if (!$.fn.DataTable.isDataTable('#datatables')) {
				$('#datatables tfoot th').each(function () {
					...
					$(this).html(
						'<input type="text" id="' +
						title +
						'" class="table-search" placeholder="Search ' +
						title +
						'"/>'
					);
				});
	...
	removeFilter(columnName, filterValue) {
		...
		const index = this.customFilters.findIndex(
			x => x.columnName === columnName && x.value === filterValue
		);
		if (index > -1) {
	...
	toggleColumn(column, cIndex) {
		if (this.selectedColumnIndex === cIndex) {
			this.columnDataType = '';
		}
		$(column)
			.closest('li')
		...
		// Hide the filters tab if it is open for this column and it is meant to be disable
		if (!this.columnNames[cIndex].showColumn && (this.selectedColumnIndex === cIndex)) {
			console.log('Applicable: ', !this.columnNames[cIndex].showColumn, this.selectedColumnIndex === cIndex);
	...
	filterN(operator) {
		...
		$.fn.dataTable.ext.search.push((settings, data, dataIndex) => {
			...
			// console.log("TCL: QueryBuilderComponent -> filterN -> this", this)
	...
	generateChart() {
		...
		const filteredDataLength = this.dTable.rows({ filter: 'applied' }).nodes()
			.length;
	...
	saveChanges() {
		...
		if (this.data) {
			...
			this.datasouceQueryService.createQuery(query).subscribe(
				res => {
